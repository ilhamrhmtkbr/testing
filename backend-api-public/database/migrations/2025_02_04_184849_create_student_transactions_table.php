...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('student_transactions', function (Blueprint $table) {
            ...
            $table->string('instructor_course_coupon_id', 70)->nullable();
            $table->integer('amount')->nullable(false);
            $table->json('midtrans_data')->nullable(false);
            $table->enum('status', ['settlement', 'deny', 'pending', 'cancel', 'expire', 'failure']);
            $table->timestamps();
            ...
            $table->foreign('instructor_course_coupon_id')->references('id')->on('instructor_courses_coupons')->onUpdate('cascade')->onDelete('cascade');
        });
    }
    ...
    {
        Schema::dropIfExists('students_transaction');
    }
};
